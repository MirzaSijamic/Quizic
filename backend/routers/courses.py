...
@router.get("/")
def get_courses(conn = Depends(get_db_connection)) -> list[CourseRead]:
    # The 'Depends' automatically opens the DB, passes it here, and closes it after.
    return course_service.fetch_courses(conn)

@router.get("/{course_id}")
def get_course_by_id(course_id: int, conn = Depends(get_db_connection)) -> CourseRead:
    course = course_service.fetch_course_by_id(conn, course_id)
    if not course:
...
def update_course(course_id: int, course_data: CourseUpdate, conn = Depends(get_db_connection)) -> CourseRead:
